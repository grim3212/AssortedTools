...
public class ToolsItemTagProvider extends LibItemTagProvider {
    ...
    public void addCommonTags(Function<TagKey<Item>, TagAppender<Item>> appender, BiConsumer<TagKey<Block>, TagKey<Item>> copier) {
        ...
        tagger.apply(ToolsEnchantments.SPEAR_ENCHANTABLE).add(ToolsItems.WOOD_THROWING_SPEAR.get(), ToolsItems.STONE_THROWING_SPEAR.get(), ToolsItems.IRON_THROWING_SPEAR.get(), ToolsItems.GOLD_THROWING_SPEAR.get(), ToolsItems.DIAMOND_THROWING_SPEAR.get(), ToolsItems.NETHERITE_THROWING_SPEAR.get());
        ...
        ToolsItems.MATERIAL_GROUPS.forEach((s, group) -> {
            ...
            // Where vanilla puts its own spears: the spear tag, piglins' preferred weapons, and the
            // lunge enchantment on top of the melee and durability ones below.
            tagger.apply(ItemTags.SPEARS).add(group.SPEAR.get());
            tagger.apply(ItemTags.PIGLIN_PREFERRED_WEAPONS).add(group.SPEAR.get());
            tagger.apply(ItemTags.LUNGE_ENCHANTABLE).add(group.SPEAR.get());
            tagger.apply(LibCommonTags.Items.TOOLS_MELEE_WEAPONS).add(group.SWORD.get(), group.AXE.get(), group.MULTITOOL.get(), group.SPEAR.get());
